...
m_psyco_prcp_r <- reactive({mpsyc %>% filter(modulo==input$moduloP2) %>% group_by(cluster,`V12. VALOR_AGUA [Considera que debemos restringirla para los ecosistemas]`) %>% summarise(Freq=n())})
...
m_psyco_m3_r <- reactive({mpsyc %>% filter(modulo==input$moduloP3) %>% group_by(cluster,`V12. VALOR_AGUA [Considera que debemos preservarla para futuras generaciones]`) %>% summarise(Freq=n())})
